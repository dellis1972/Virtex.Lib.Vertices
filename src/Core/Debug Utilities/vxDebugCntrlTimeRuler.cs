...
namespace Virtex.Lib.Vrtc.Core.Debug
{
	...
	public class vxDebugCntrlTimeRuler : DrawableGameComponent
    {
        ...
        {
#if TRACE
			Width = (int)(GraphicsDevice.Viewport.Width * 0.65f);
            ...
            foreach (MarkerCollection bar in prevLog.Bars)
            {
                ...
                if (bar.MarkCount > 0)
                {
                    for (int j = 0; j < bar.MarkCount; ++j)
                    {
						...
						runningmax = 50;//Math.Max (time, runningmax);
		...
		public void DrawGraph()
		{
			...
			if (ShowGraph && ShowLog && vxCrashHandler.IsInitialised == false) {
				...
				debugManager.vxEngine.SpriteBatch.Draw (debugManager.vxEngine.Assets.Textures.Blank, 
					new Rectangle (
						rcnew.X - buffer, 
						rcnew.Y - buffer, 
					...
					new Color (0, 0, 0, 150));
				debugManager.vxEngine.SpriteBatch.DrawString(debugManager.DebugFont, runningmax.ToString(), new Vector2 (rcnew.Right + 5, rcnew.Y), Color.White);
				debugManager.vxEngine.SpriteBatch.DrawString(debugManager.DebugFont, 0.ToString(), new Vector2 (rcnew.Right + 5, rcnew.Bottom - debugManager.DebugFont.MeasureString("D").Y), Color.White);



				debugManager.vxEngine.SpriteBatch.End ();
