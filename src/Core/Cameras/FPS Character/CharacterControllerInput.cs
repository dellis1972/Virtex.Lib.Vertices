...
namespace vxVertices.Core.Cameras.Controllers
{
    ...
    public class CharacterControllerInput
    {
        ...
        /// <summary>
        /// Engine stores settings like Key Bindings, Mouse Sensitvity etc...
        /// </summary>
        ...
        public CharacterControllerInput(Engine engine, Space owningSpace, FPSCamera cameraToUse)
        {
        ...
        public void Update(float dt, KeyboardState previousKeyboardInput, KeyboardState keyboardInput, GamePadState previousGamePadInput, GamePadState gamePadInput)
        {
            if (IsActive)
            {
                ...
                //Move Forward
                if (keyboardInput.IsKeyDown(Engine.KeyBinding_Forward))
                {
                ...
                //Move Backwards
                if (keyboardInput.IsKeyDown(Engine.KeyBinding_Back))
                {
                    movementDir = Camera.WorldMatrix.Forward;
                    totalMovement -= Vector2.Normalize(new Vector2(movementDir.X, movementDir.Z));
                }
                ...
                //Move Sideways Left
                if (keyboardInput.IsKeyDown(Engine.KeyBinding_Left))
                {
                ...
                //Move Sideways Right
                if (keyboardInput.IsKeyDown(Engine.KeyBinding_Right))
                {
                ...
                CharacterController.StanceManager.DesiredStance = keyboardInput.IsKeyDown(Engine.KeyBinding_Croutch) ? Stance.Crouching : Stance.Standing;

                //Jumping
                if (previousKeyboardInput.IsKeyUp(Engine.KeyBinding_Jump) && keyboardInput.IsKeyDown(Engine.KeyBinding_Jump))
                {
                    CharacterController.Jump();
                }
#endif
