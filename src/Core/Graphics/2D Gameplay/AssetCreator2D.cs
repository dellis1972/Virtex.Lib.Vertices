...
namespace Virtex.Lib.Vrtc.Graphics
{
	...
	public class AssetCreator2D
	{
		...
		{
			return TextureFromVertices (vertices, type, color, materialScale, Color.Black);
		}

		public Texture2D TextureFromVertices(Vertices vertices, MaterialType type, Color color, float materialScale, Color EdgeColor)
		{
			// copy vertices
			...
			for (int i = 0; i < verts.Count; ++i)
			{
				...
				verticesOutline[2 * i].Color = verticesOutline[2 * i + 1].Color = EdgeColor;
			}
